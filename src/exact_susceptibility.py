...
def nonlocal_susceptibility(r, rp, Z):
    ...
    return (
        h_atom_groundstate(np.linalg.norm(r), Z)
        * h_atom_groundstate(np.linalg.norm(rp), Z)
...
def white_formula(r, rp, Z):
    ...
    second_term = (
        np.euler_gamma - 5 / 2 + Z * (r1 + r2) + np.log(Z * (r1 + r2 + r12)) - integral
    )

    return (
        h_atom_groundstate(r1, Z)
        * h_atom_groundstate(r2, Z)
        * (first_term + second_term_prefactor * second_term)
    )
