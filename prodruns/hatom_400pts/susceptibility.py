...
def real_space_scan():
    ...
    # random grid subset
    npts = coords.shape[0]
    idx = np.random.choice(npts, 400, replace=False)
...
outfile.write('H atom, 400 pts for chi, pi/4 angle, linspace(0.2, 2)')
...
for pairs in it.product(distance_range, repeat=2):
    ...
    outfile.write(str(pairs[0]) + ' ' +str(pairs[1]) + ' '+str( np.trace(pol)) + '\n')
