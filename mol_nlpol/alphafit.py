...
import molecular_susceptibility as molsus
#import quadpy
import os.path


#############################################################################################
# General Legendre polynomial class for fitting; no need to touch
#############################################################################################
class Legendre1D:
...
class PolarizabilityBasis:
    def __init__(
        ...
        self._derivs = []
        for derivative in derivatives: # Removes the constant term from fitting?
            if derivative == np.poly1d([0]):
                derivative = None # Why not felete from basis too?
    ...
    def fit_ao_basis(self, points):
        ...
        for i, j in it.product(range(3), range(3)):
            ...
            for r, rprime in tqdm.tqdm(
                ...
                if i == 0 and j == 0:
                    ys[row] = molsus.mol_sus(
                        points[r], points[rprime]
                    ) # Build the susceptibility, but only once in the loop...

                left = derivs[i + 1, r, :] # Why +1? To remove the constant again? Isnt it removed?
        ...
        # filter
        mask = np.isfinite(ys) # Sometimes the reference is not finite
        ys = ys[mask]
        A = A[mask, :]
    ...
    def fit(self, points: Iterable[Iterable[float]], susceptibilities: Iterable[float]): # Is this an alternative implementation?
        xs = np.array(points)
...
def test(N):
    ...
    for last in range(len(chi)):
        chi[last] = molsus.mol_sus(points[last, :3], points[last, 3:])
...
def do_single_panel(solution): # A single plot in the Z=0 plane
    f = plt.figure()
    ax = f.add_axes( [0.1, 0.1, 0.8, 0.8] )

    N = 100
    mags = np.linspace(-2, 2, N)
    rs, rps = np.zeros((N * N, 3)), np.zeros((N * N, 3))
    xs = np.repeat(mags, N)
    ys = np.tile(mags, N)
    rs[:, 0] = 0.0
    ...
    P = PolarizabilityBasis([], [])
    P.response_basis_set("def2-TZVP", "C", 2)
    values = to_value(P._molresp, rs, rps, 1, 1, solution)
    # plt.imshow(values.reshape(N, N), cmap="Blues", vmin=-5)
    # levels = np.percentile(values, np.arange(0, 110, 10))
    levels = np.linspace(np.percentile(values, 0.1), max(values), 12)
...
if __name__ == "__main__":
    ...
    N = 15
    fnam = str(N) + ".npy"
    if os.path.isfile(fnam): # Saving the solution to mess with plotting
        solution = np.load(fnam)
    else:
        solution = test(N)
        np.save(fnam, solution)
    do_single_panel(solution)
    # profiler.stop()
